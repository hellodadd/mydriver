...
import com.droi.account.widget.wheel.adapters.AbstractWheelTextAdapter;
import com.droi.account.widget.wheel.adapters.WheelViewAdapter;
...
public class WheelView extends View {
	...
	// Center Line
	private Drawable centerDrawable = null;
	...
	private DataSetObserver dataObserver = new DataSetObserver() {
		...
		public void onChanged() {
			if(DebugUtils.DEBUG && getId() == mMyResources.getId("lib_droi_account_id_province")){
				DebugUtils.i(TAG, "onChanged false ");
			}
		...
		public void onInvalidated() {
			if(DebugUtils.DEBUG && getId() == mMyResources.getId("lib_droi_account_id_province")){
				DebugUtils.i(TAG, "onInvalidated true");
			}
	...
	public void setCyclic(boolean isCyclic) {
		...
		if(DebugUtils.DEBUG && getId() == mMyResources.getId("lib_droi_account_id_province")){
			DebugUtils.i(TAG, "setCyclic false");
		}
		invalidateWheel(false);
	}
	...
	public void invalidateWheel(boolean clearCaches) {
		if(DebugUtils.DEBUG && getId() == mMyResources.getId("lib_droi_account_id_province")){
			DebugUtils.i(TAG, "invalidateWheel clearCaches : " + clearCaches);
		}
		if (clearCaches) {
			recycle.clearAll();
			if (itemsLayout != null) {
	...
	private void drawCenterRect(Canvas canvas) {
		...
		//centerDrawable.setBounds(-3, center - offset, getWidth() + 3, center + offset);
	...
	private void buildViewForMeasuring() {
		// clear all items
		if(DebugUtils.DEBUG && getId() == mMyResources.getId("lib_droi_account_id_province")){
			DebugUtils.i(TAG, "buildViewForMeasuring");
		}
		if (itemsLayout != null) {
			recycle.recycleItems(itemsLayout, firstItem, new ItemsRange());
		} else {
		...
		if (view != null) {
			...
			if(itemsLayout != null){
				if(view != null && (view instanceof TextView)){
					...
					if(index == currentItem){
						text.setTextColor(AbstractWheelTextAdapter.TEXT_COLOR_BLACK);
					}else{
						text.setTextColor(AbstractWheelTextAdapter.TEXT_COLOR_GRAY);
					}
				}
			}
			//sean end
	...
	//sean begin
	public void updateTextColor(String centerString){
		if(itemsLayout != null){
			//int centerIndex = getCenterViewIndex(centerString);
			for(int i = 0; i < itemsLayout.getChildCount(); i++){
				View child = itemsLayout.getChildAt(i);
				if(child != null && (child instanceof TextView)){
					TextView text = (TextView)child;
					if(centerString != null && centerString.equals(text.getText())){
						text.setTextColor(AbstractWheelTextAdapter.TEXT_COLOR_BLACK);
					}else{
						text.setTextColor(AbstractWheelTextAdapter.TEXT_COLOR_GRAY);
					}
	...
	private View getItemView(int index) {
		...
		if(DebugUtils.DEBUG && getId() == mMyResources.getId("lib_droi_account_id_province")){
			...
			if(view instanceof TextView){
				DebugUtils.i(TAG, "getItemView index = " + index + ", text = " + ((TextView)view).getText());
			}
		}
